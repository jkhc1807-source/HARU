...
type RouteSuggestion = { optimized: Spot[]; savedMinutes: number };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setRouteSuggestion(null);
    };
  ...
  function addSpot(spot: Spot) {
    ...
    suggestShorterRoute(nextPlan);
  }

  function suggestShorterRoute(nextPlan: Spot[]) {
    const optimized = optimizeRoute(nextPlan);
    const savedMinutes = routeTravelMinutes(nextPlan) - routeTravelMinutes(optimized);
    if (savedMinutes < 5 || optimized.map(spot => spot.id).join(",") === nextPlan.map(spot => spot.id).join(",")) return;
    setRouteSuggestion({ optimized, savedMinutes });
  }
  ...
  function handleDropAt(index: number) {
    ...
    if (previousIndex < 0) suggestShorterRoute(nextPlan);
    setDraggedSpot(null);
    setIsDragging(false);
  }
  ...
  return (
    <main>
      ...
      {routeSuggestion && <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setRouteSuggestion(null); }}>
        <div className="route-modal" role="dialog" aria-modal="true" aria-labelledby="route-modal-title">
          <span className="route-modal-icon" aria-hidden="true">↗</span>
          <p className="eyebrow">SHORTER ROUTE</p>
          <h2 id="route-modal-title">더 짧은 동선을 찾았어요</h2>
          <p>방문 순서를 바꾸면 예상 도보시간을 약 <b>{routeSuggestion.savedMinutes}분</b> 줄일 수 있어요.</p>
          <div className="route-preview">{routeSuggestion.optimized.map((spot, index) => <Fragment key={spot.id}><span>{spot.name}</span>{index < routeSuggestion.optimized.length - 1 && <i>→</i>}</Fragment>)}</div>
          <div className="modal-actions">
            <button type="button" className="modal-secondary" onClick={() => setRouteSuggestion(null)}>현재 순서 유지</button>
            <button type="button" className="modal-primary" ref={optimizeButtonRef} onClick={() => {
              setPlan(routeSuggestion.optimized);
              setNotice(`동선을 최적화해 도보시간을 약 ${routeSuggestion.savedMinutes}분 줄였어요`);
