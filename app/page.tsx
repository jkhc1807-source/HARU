            ...
            {schedule.map(({ spot, start, end, travelToNext }, i) => <Fragment key={spot.id}>
              ...
              <article className={`stop tone-${toneForSpot(spot)} ${i === schedule.length - 1 ? "last" : ""}`} data-stop-index={i} onPointerDown={event => { if (event.pointerType !== "touch" && !(event.target as HTMLElement).closest("a, button, select")) handlePointerDragStart(event, spot); }} aria-label={`${spot.name} 일정 순서 이동`}>
                <div className="stop-card compact-stop">
                  ...
                  <details className="stop-actions" onPointerDown={event => event.stopPropagation()} onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); event.currentTarget.open = false; event.currentTarget.querySelector("summary")?.focus(); } }}>
                    ...
                    <div className="stop-actions-panel">
                      ...
                      <details className="spot-editor" onPointerDown={event => event.stopPropagation()}><summary>시간·메모 수정</summary><div className="spot-editor-fields"><span className="stay-control">체류 <ChoiceSelect className="stay-choice" value={String(spot.stay)} placeholder={`${spot.stay}분`} ariaLabel={`${spot.name} 체류 시간`} options={stayOptions.map(option => ({ value: String(option), label: `${option}분` }))} onChange={value => handleStayChange(i, Number(value))} /></span><label className="spot-note">메모<textarea rows={1} value={spot.note || ""} maxLength={160} placeholder="예: 웨이팅 확인, 꼭 먹을 메뉴" aria-label={`${spot.name} 메모`} onChange={event => { event.currentTarget.style.height = "auto"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 120)}px`; handleNoteChange(spot.id, event.target.value); }} ref={element => { if (element) { element.style.height = "auto"; element.style.height = `${Math.min(element.scrollHeight, 120)}px`; } }} /></label></div></details>
