...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setToast(null), undo ? 7000 : 4000);
    return () => window.clearTimeout(timer);
  }, [notice]);

  // 헤더 드롭다운은 Escape 와 바깥 클릭으로 닫는다. 카드 ⋯ 메뉴와 동작을 맞춘다.
  useEffect(() => {
    const menus = () => [...document.querySelectorAll<HTMLDetailsElement>("details.header-more-actions[open], details.auth-user[open]")];
  ...
  useEffect(() => {
    ...
    // 시간을 바꿨다고 일정이 통째로 사라지면 놀란다. 그때는 그대로 두고
    // 초과 경고와 '시간에 맞게 줄이기' 로 사용자가 직접 정하게 한다.
    if (!fitted.length) return;
    // 여기서는 updatePlan 을 쓰지 않는다. 그 안의 setIsPlanAutoGenerated(false) 가
    // 이후 자동 재조정을 멈춰버리기 때문이다. 되돌리기 상태만 따로 남긴다.
    setUndoState({ plan, message: "시간에 맞춘 일정 조정" });
    toastUndoRef.current = true;
    setPlan(fitted);
  ...
  function updatePlan(nextPlan: Spot[], undoMessage: string) {
    ...
    setIsPlanAutoGenerated(false);
    setPlan(nextPlan);
    // 장소 삭제처럼 알림을 따로 남기지 않던 동작도 결과가 보이게 한다.
    // 호출한 쪽에서 뒤이어 setNotice 를 하면 그 문구가 이긴다.
    toastUndoRef.current = true;
    setNotice(`${undoMessage} · 되돌릴 수 있어요`);
  }
  ...
  function handleFitToTime() {
    ...
    // 줄이려고 눌렀는데 전부 사라지는 건 예상 밖이다. 그 경우에만 먼저 묻는다.
    if (!fitted.length && plan.length
      && !window.confirm(`${startTime}–${endTime} 안에 들어가는 장소가 없어 일정이 비워져요. 계속할까요?`)) return;
  ...
  function searchPlaces(e: React.FormEvent) {
    ...
    // 사용자가 이미 지역명을 넣었으면 또 붙이지 않는다 ('성수동 성수동 카페' 처럼 보였다)
    const term = region && !keyword.includes(region) ? `${region} ${keyword}`.trim() : keyword.trim();
    if (mapReady && window.kakao?.maps?.services) {
  ...
  return (
    <main>
      ...
      {toast && <div className="toast" role="status" aria-live="polite">
        ...
        {toast.undo && undoState && <button type="button" onClick={() => { handleUndo(); setToast(null); }}>↶ 실행 취소</button>}
      </div>}
